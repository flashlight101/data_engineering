...
def get_viper_id(browser, link):
    ...
    radius_inner = re.findall(r'(?<=Inner:.)([0-9]*Å)(?=.)',browser.find_element(By.XPATH,f"//*[@id=\"Rad\"]").text)[0] 
    ...
    diameter_inner = re.findall(r'(?<=Inner:.)([0-9]*Å)(?=.)',browser.find_element(By.XPATH,f"//*[@id=\"Diam\"]").text)[0]
    diameter_ave = re.findall(r'(?<=Ave:.)([0-9]*Å)(?=.)',browser.find_element(By.XPATH,f"//*[@id=\"Diam\"]").text)[0]
    diameter_outer = re.findall(r'(?<=Outer:.)([0-9]*Å)(?=)',browser.find_element(By.XPATH,f"//*[@id=\"Diam\"]").text)[0]
...
def get_pdb_id(browser, link):
    ...
    try:
        image_bottom_inf = browser.find_element(By.XPATH,f"//*[@id=\"Carousel-BiologicalUnit1\"]/div[4]").text.splitlines()
    except:
        return None
    ...
    total_structure_weight = re.findall(r'(?<=: )([^#]+)(?= )',browser.find_element(By.XPATH,f"//*[@id=\"contentStructureWeight\"]").text)
    atom_count = re.findall(r'(?<=: )([^#]+)(?= )',browser.find_element(By.XPATH,f"//*[@id=\"contentAtomSiteCount\"]").text)[0].replace(',','.')
    modelled_residue_count = re.findall(r'(?<=: )([^#]+)(?= )',browser.find_elements(By.XPATH,f"//*[@id=\"contentResidueCount\"]")[0].text)[0]
    deposited_residue_count = re.findall(r'(?<=: )([^#]+)(?= )',browser.find_elements(By.XPATH,f"//*[@id=\"contentResidueCount\"]")[1].text)[0]
    unique_protein_chains = re.findall(r'(?<=: )([^#]+)(?=)',browser.find_element(By.XPATH,f"//*[@id=\"contentProteinChainCount\"]").text)[0]
    try:
...
def get_all_virus(browser):
    ...
    while True:
        ...
        while True:
            try:
                url = browser.find_element(By.XPATH,f"//*[@id=\"table\"]/tr[{row}]/td[2]/div[2]/a").get_attribute("href")
                row += 1
                url_list.append(url)
            except:
                break      
